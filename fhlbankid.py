...
# plt.title('Percentage of Missing Values by Column', fontsize=13 )
# plt.ylabel('Percentage of Missing Values')
# plt.xticks(rotation=90)
# plt.show()

# missing_values_by_year = df.groupby('Year').apply(lambda x: x.isna().sum())
#
# # Calculate the percentage of missing values for each column
# percent_missing = missing_values_by_year.transform(lambda x: x * 100 / len(x))
#
# # Plot the results
# plt.plot(percent_missing.index, percent_missing['column_name'], marker='o')
# plt.title('Percentage of Missing Values by Year')
# plt.ylabel('Percentage of Missing Values')
# plt.xticks(rotation=90)
# plt.show()
#
# df.drop(['LoanNumber', 'AssignedID','Coop','Program','AcquDate','MortDate','Bed1','Bed2','Bed3','Bed4','Aff1','Aff2','Aff3','Aff4','Occup','NumUnits','RentUt1','RentUt2','RentUt3','Product','RentUt4','Rent1','Rent2','Rent3','Rent4','FeatureID','SellType','Seller','SellCity','SellSt','CICA','LienStatus','FedFinStbltyPlan','GSEREO','FHFBID','Geog','SpcHsgGoals','AcqTyp','PrepayP','ArmIndex','AmorTerm','Corace5','Corace4','Corace3','Race3','Race4','Race5'], axis=1, inplace=True)
# corr_matrix = df.corr()
...
# # df['PrepayP'] = pd.to_datetime(df['PrepayP'], errors='coerce').fillna(0)
# # df['PrepayP'] = pd.to_numeric(pd.to_datetime(df['PrepayP'], errors='coerce').fillna(0))
